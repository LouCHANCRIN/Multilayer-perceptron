...
def cost_function(Y, W, B, A_test, Z_test, activation, hyp):
    forward(A_test, Z_test, W, B, activation, hyp)
    ret = 0
    YH = A_test[hyp.nb_layer]
    y, x = np.shape(YH)
    #print(np.shape(Y), np.shape(YH))
    if (activation[hyp.nb_layer] == "soft_max"):
        for i in range(0, y):
            for j in range(0, x):
                if (Y[i][j] == 1):
    ...
    if (hyp.lambd != 1):
        ...
        for i in range(1, nb_layer + 1):
            ...
            return ((-ret / (x * y)) + ((hyp.lambd / (2 * x)) * regu))

def gradient_checking(W, B, DW, DB, Y_test, line_test, A_test, Z_test, hyp):
    size = 0
    for i in range(1, hyp.nb_layer + 1):
        x, y = np.shape(W[i])
        size += x * y + x
    ...
    for i in range(1, hyp.nb_layer + 1):
        ...
        for j in range(0, x):
            for k in range(0, y):
                W[i][j][k] += eps
                cost1 = cost_function(Y_test, W, B, A_test, Z_test, activation, hyp)
                W[i][j][k] -= (2 * eps)
                cost2 = cost_function(Y_test, W, B, A_test, Z_test, activation, hyp)
                W[i][j][k] += eps
...
def forward(A, Z, W, B, activation, hyp):
    for l in range(1, hyp.nb_layer + 1):
...
def backward(A, DA, W, DW, B, DB, Z, DZ, Y, activation, hyp):
    l = hyp.nb_layer
    DZ[l] = A[l] - Y
    ...
    if (hyp.gradient == 1):
        gradient_checking(W, B, DW, DB, Y, line_train, A, Z, hyp)
    if (hyp.momentum != 1):
        for i in range(1, hyp.nb_layer + 1):
            W[i] = W[i] - hyp.alpha * (DW[i] + ((hyp.lambd / (2 * line_train)) * W[i]))
            B[i] = B[i] - hyp.alpha * DB[i]
    else:
        for i in range(1, hyp.nb_layer + 1):
            x, y = np.shape(W[i])
            ...
            vdw = (hyp.beta * vdw) + ((1 - hyp.beta) * DW[i])
            vdb = (hyp.beta * vdb) + ((1 - hyp.beta) * DB[i])
            W[i] = W[i] - (hyp.alpha * vdw)
            B[i] = B[i] - (hyp.alpha * vdb)

def neural_network(Y, Y_test, W, cost, B, activation, hyp):
    for i in range(0, hyp.num_iters):
        if (i % 100 == 0):
            #alpha *= 0.98
            print(i)
        forward(A, Z, W, B, activation, hyp)
        backward(A, DA, W, DW, B, DB, Z, DZ, Y, activation, hyp)
        if (hyp.gradient == 1):
            hyp.gradient = 0
        cost.append(cost_function(Y_test, W, B, A_test, Z_test, activation, hyp))
        if (cost[i] > cost[i - 1]):
            print(i + 1)
            A[0] = X_test
            forward(A, Z, W, B, activation, hyp)
            return (A[hyp.nb_layer], i)
    A[0] = X_test
    forward(A, Z, W, B, activation, hyp)
    return (A[hyp.nb_layer], hyp.num_iters)

############## INITIALISATION #################


A = [0] * (hyp.nb_layer + 1)
A_test = [0] * (hyp.nb_layer + 1)
Z = [0] * (hyp.nb_layer + 1)
Z_test = [0] * (hyp.nb_layer + 1)
DA = [0] * (hyp.nb_layer + 1)
DW = [0] *(hyp.nb_layer + 1 )
DZ = [0] * (hyp.nb_layer + 1)
DB = [0] * (hyp.nb_layer + 1)
...
def main():
    if (activation[hyp.nb_layer] == "soft_max"):
        YH, num_iters = neural_network(Y_soft, Y_soft_test, W, cost, B, activation, hyp)
    else:
        YH, num_iters = neural_network(Y, Y_test, W, cost, B, activation, hyp)
    YH = np.reshape(YH, (n[hyp.nb_layer], line_test))
    return (YH, num_iters)
